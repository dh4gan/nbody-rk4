...
class orbitalElements(object):
    ...
    def __init__(self,a, e, i, longascend, argper, trueanom, position,velocity, G, totalMass):
        ...
        self.angmom = 0
        self.angmomvec = Vector3D(0.0,0.0,0.0)
    ...
    def calcOrbitFromVector(self):
        ...
        self.angmomvec = self.position.cross(self.velocity)
        self.angmom = self.angmomvec.mag()
        ...
        if (self.angmom > 0.0):
            self.i = np.arccos(self.angmomvec.z/self.angmom)   
        else:
            self.i = 0.0 
        ...
        nplane = zhat.cross(self.angmomvec)
